...
export default function Home() {
  ...
  const [adminTab, setAdminTab] = useState<"profile" | "project" | "music">("project");
  ...
  const [formMusicTitle, setFormMusicTitle] = useState("");
  const [formMusicArtist, setFormMusicArtist] = useState("");
  const [formMusicUrl, setFormMusicUrl] = useState("");
  const [formMusicCoverArt, setFormMusicCoverArt] = useState("");
  const [formMusicDate, setFormMusicDate] = useState("");
  const [musicImageMode, setMusicImageMode] = useState<"url" | "upload">("url");
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [projectsRes, profileRes, musicsRes] = await Promise.all([
          fetch("/api/projects"),
          fetch("/api/profile"),
          fetch("/api/music")
        ]);
        ...
        if (musicsRes.ok) {
          const data = await musicsRes.json();
          if (Array.isArray(data)) {
            setMusics(data);
          } else {
            setMusics([]);
  ...
  const handleSaveMusic = async (e: React.FormEvent) => {
    ...
    const musicData = {
      title: formMusicTitle,
      artist: formMusicArtist,
      url: formMusicUrl,
      coverArt: formMusicCoverArt,
      date: formMusicDate,
    };

    try {
      const method = editingMusicId ? "PUT" : "POST";
      const body = editingMusicId ? { id: editingMusicId, ...musicData } : musicData;
      ...
      if (response.ok) {
        const data = await response.json();
        if (editingMusicId) {
          setMusics(musics.map((m) => (m.id === editingMusicId ? data.music : m)));
        } else {
          setMusics([...musics, data.music]);
        }
        cancelEditMusic();
  ...
  const startEditMusic = (music: Music) => {
    setAdminTab("music");
    setEditingMusicId(music.id);
    setFormMusicTitle(music.title || "");
    setFormMusicArtist(music.artist || "");
    setFormMusicUrl(music.url || "");
    setFormMusicCoverArt(music.coverArt || "");
    setFormMusicDate(music.date || "");
    setMusicImageMode(music.coverArt?.startsWith("data:image") ? "upload" : "url");
    window.scrollTo({ top: document.body.scrollHeight, behavior: "smooth" });
  };
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-gray-300 font-sans selection:bg-cyan-500/30 selection:text-cyan-200 pb-24 overflow-x-hidden">
      ...
      <header className="flex justify-between items-center p-4 md:p-6 border-b border-gray-800/50 backdrop-blur-md sticky top-0 z-50 bg-[#050505]/80">
            ...
            <div className="flex items-center gap-2">
              {showLogin && (
                <form onSubmit={handleLogin} className="flex">
                  <input type="password" autoFocus value={passwordInput} onChange={(e) => setPasswordInput(e.target.value)} className="bg-gray-900 border border-gray-800 text-sm px-3 py-1.5 rounded-l-md focus:outline-none focus:border-cyan-500 transition w-24 md:w-32" placeholder="password" />
      ...
      <main className="max-w-7xl mx-auto px-4 md:px-6 py-12 md:py-20 relative z-10">
        <section className="flex flex-col md:flex-row items-center justify-between mb-24 md:mb-32 gap-12">
          {isLoading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {Array.isArray(projects) && projects.filter(p => p != null).map((project) => (
                <div key={project.id} className="group relative bg-[#0a0a0a] border border-gray-800 rounded-2xl overflow-hidden hover:border-gray-600 transition-all flex flex-col h-full">
                  <div className="w-full h-48 md:h-56 relative bg-gray-950 shrink-0 border-b border-gray-800/50 overflow-hidden">
        ...
        <section className="mb-24">
          <div className="mb-12 flex flex-col items-center text-center">
            ...
            <h3 className="text-3xl md:text-5xl font-black text-white">
              MUSIC <span className="text-gray-500 font-light">COLLECTION</span>
            </h3>
          </div>
          ...
          {isLoading ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {[1, 2, 3, 4].map((i) => (
                <div key={i} className="h-64 bg-gray-900 border border-gray-800 rounded-2xl animate-pulse"></div>
              ))}
            </div>
          ) : musics.length === 0 ? (
            <div className="py-24 border border-dashed border-gray-800 rounded-2xl flex flex-col items-center justify-center text-gray-500 bg-gray-900/20">
              <span className="text-sm">No music tracks uploaded yet.</span>
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {Array.isArray(musics) && musics.filter(m => m != null).map((music) => (
                <div key={music.id} className="group relative bg-[#0a0a0a] border border-gray-800 rounded-2xl overflow-hidden hover:border-gray-600 transition-all flex flex-col">
                  <div className="w-full aspect-square relative bg-gray-950 shrink-0 border-b border-gray-800/50 overflow-hidden">
                    {music.coverArt ? (
                      <Image src={music.coverArt} alt={music.title || "Cover Art"} fill sizes="(max-width: 768px) 100vw, 25vw" className="object-cover opacity-90 group-hover:opacity-100 group-hover:scale-105 transition-all duration-500" />
                    ) : (
                      <div className="flex items-center justify-center h-full w-full text-gray-700">
                        <svg className="w-12 h-12" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 19V6l12-3v13M9 19c0 1.105-1.343 2-3 2s-3-.895-3-2 1.343-2 3-2 3 .895 3 2zm12-3c0 1.105-1.343 2-3 2s-3-.895-3-2 1.343-2 3-2 3 .895 3 2zM9 10l12-3"></path></svg>
                      </div>
                    )}
                    
                    {music.url && (
                      <a href={music.url} target="_blank" rel="noreferrer" className="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                         <div className="bg-white text-black rounded-full p-4 transform scale-90 group-hover:scale-100 transition-transform shadow-lg">
                  ...
                  <div className="p-4 flex flex-col justify-center text-center">
                    <h4 className="text-base font-bold text-white truncate">{music.title || "Unknown Track"}</h4>
                    <p className="text-gray-400 text-xs mt-1 truncate">{music.artist || "Unknown Artist"}</p>
                    {music.date && <span className="text-gray-600 text-[10px] font-mono mt-2">{music.date}</span>}
                  </div>

                  {isAdmin && (
                    <div className="absolute top-2 right-2 flex gap-1 z-20 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button onClick={() => startEditMusic(music)} className="bg-white hover:bg-gray-200 text-black p-1.5 rounded-full shadow-lg transition">
                        <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
                      </button>
                      <button onClick={() => handleDeleteMusic(music.id)} className="bg-red-500 hover:bg-red-600 text-white p-1.5 rounded-full shadow-lg transition">
        ...
        {isAdmin && (
          <div className="mt-20 md:mt-32 relative border-t border-gray-800 pt-10 md:pt-16 px-2 md:px-0">
            <div className="flex items-center justify-center mb-8 md:mb-10">
              <div className="bg-gray-900 border border-gray-800 rounded-full p-1 flex flex-wrap justify-center gap-1 w-full max-w-md md:max-w-none md:w-auto">
                <button onClick={() => setAdminTab("profile")} className={`px-4 md:px-6 py-2 text-xs md:text-sm font-medium rounded-full transition-all flex-1 md:flex-none ${adminTab === "profile" ? "bg-white text-black shadow-lg" : "text-gray-400 hover:text-white"}`}>
                  Profile
                </button>
                <button onClick={() => setAdminTab("project")} className={`px-4 md:px-6 py-2 text-xs md:text-sm font-medium rounded-full transition-all flex-1 md:flex-none ${adminTab === "project" ? "bg-white text-black shadow-lg" : "text-gray-400 hover:text-white"}`}>
                  Projects
                </button>
                <button onClick={() => setAdminTab("music")} className={`px-4 md:px-6 py-2 text-xs md:text-sm font-medium rounded-full transition-all flex-1 md:flex-none ${adminTab === "music" ? "bg-white text-black shadow-lg" : "text-gray-400 hover:text-white"}`}>
                  Music
                </button>
              </div>
            </div>
            ...
            {adminTab === "profile" && (
              <section className="bg-[#0a0a0a] border border-gray-800 p-4 md:p-10 rounded-3xl shadow-2xl relative">
                <h2 className="text-xl md:text-2xl font-bold mb-6 md:mb-8 text-white">Profile Configuration</h2>

                <form onSubmit={handleUpdateProfile} className="space-y-6">
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
                    <div className="space-y-2">
                      <label className="text-[10px] md:text-xs font-bold tracking-wider text-gray-500 uppercase">Greeting</label>
                    ...
                    <div className="space-y-2">
                      <label className="text-[10px] md:text-xs font-bold tracking-wider text-gray-500 uppercase">Full Name</label>
                    ...
                    <div className="space-y-2">
                      <label className="text-[10px] md:text-xs font-bold tracking-wider text-gray-500 uppercase">Main Title</label>
                    ...
                    <div className="space-y-2">
                      <label className="text-[10px] md:text-xs font-bold tracking-wider text-gray-500 uppercase">Subtitle</label>
                  ...
                  <div className="p-4 md:p-5 bg-gray-900/50 border border-gray-800 rounded-2xl space-y-4">
                    <h3 className="text-xs md:text-sm font-bold text-gray-400">Portfolio Section Text</h3>
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 md:gap-6">
                      <div className="space-y-2">
                        <label className="text-[10px] md:text-xs font-bold tracking-wider text-gray-500 uppercase">Small Label</label>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                    <div className="space-y-2">
                      <label className="text-[10px] md:text-xs font-bold tracking-wider text-gray-500 uppercase">Tech Stack (Comma Separated)</label>
                    ...
                    <div className="space-y-2">
                      <label className="text-[10px] md:text-xs font-bold tracking-wider text-gray-500 uppercase">Tools & Platforms (Comma Separated)</label>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                    <div className="space-y-2">
                      <label className="text-[10px] md:text-xs font-bold tracking-wider text-gray-500 uppercase">Email</label>
                    ...
                    <div className="space-y-2">
                      <label className="text-[10px] md:text-xs font-bold tracking-wider text-gray-500 uppercase">Phone</label>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
                    <div className="space-y-2">
                      <label className="text-[10px] md:text-xs font-bold tracking-wider text-gray-500 uppercase">GitHub URL</label>
                    ...
                    <div className="space-y-2">
                      <label className="text-[10px] md:text-xs font-bold tracking-wider text-gray-500 uppercase">LinkedIn URL</label>
                    ...
                    <div className="space-y-2">
                      <label className="text-[10px] md:text-xs font-bold tracking-wider text-gray-500 uppercase">Twitter URL</label>
                    ...
                    <div className="space-y-2">
                      <label className="text-[10px] md:text-xs font-bold tracking-wider text-gray-500 uppercase">Resume Link</label>
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between items-center">
                      <label className="text-[10px] md:text-xs font-bold tracking-wider text-gray-500 uppercase">Avatar Image</label>
                  ...
                  <div className="space-y-2">
                    <label className="text-[10px] md:text-xs font-bold tracking-wider text-gray-500 uppercase">Bio / About</label>
            ...
            {adminTab === "project" && (
              <section className="bg-[#0a0a0a] border border-gray-800 p-4 md:p-10 rounded-3xl shadow-2xl relative">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 md:mb-8 gap-4">
                  <h2 className="text-xl md:text-2xl font-bold text-white">{editingProjectId ? "Edit Project" : "Add New Project"}</h2>
                  {editingProjectId && (
                    <button type="button" onClick={cancelEditProject} className="text-sm bg-gray-800 text-gray-300 hover:text-white px-4 py-2 rounded-lg transition">
                      Cancel
                    </button>
                  )}
                </div>
                ...
                <form onSubmit={handleSaveProject} className="space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
                    <div className="space-y-2">
                      <label className="text-[10px] md:text-xs font-bold tracking-wider text-gray-500 uppercase">Project Title</label>
                    ...
                    <div className="space-y-2">
                      <label className="text-[10px] md:text-xs font-bold tracking-wider text-gray-500 uppercase">Your Role</label>
                    ...
                    <div className="space-y-2">
                      <label className="text-[10px] md:text-xs font-bold tracking-wider text-gray-500 uppercase">Date / Year</label>
                  ...
                  <div className="p-4 md:p-5 bg-gray-900/50 border border-gray-800 rounded-2xl space-y-4">
                    <h3 className="text-xs md:text-sm font-bold text-gray-400">Project Links & Resources</h3>
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-5">
                      <div className="space-y-2">
                        <label className="text-[10px] md:text-xs font-bold tracking-wider text-gray-500 uppercase">Live Site URL</label>
                      ...
                      <div className="space-y-2">
                        <label className="text-[10px] md:text-xs font-bold tracking-wider text-gray-500 uppercase">GitHub Repo</label>
                      ...
                      <div className="space-y-2">
                        <label className="text-[10px] md:text-xs font-bold tracking-wider text-gray-500 uppercase">Interactive Demo</label>
                      ...
                      <div className="space-y-2">
                        <label className="text-[10px] md:text-xs font-bold tracking-wider text-gray-500 uppercase">Case Study</label>
                      ...
                      <div className="space-y-2">
                        <label className="text-[10px] md:text-xs font-bold tracking-wider text-gray-500 uppercase">Figma URL</label>
                  ...
                  <div className="space-y-2">
                    <label className="text-[10px] md:text-xs font-bold tracking-wider text-gray-500 uppercase">Tech Stack (Tags, Comma Separated)</label>
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between items-center">
                      <label className="text-[10px] md:text-xs font-bold tracking-wider text-gray-500 uppercase">Cover Image</label>
                  ...
                  <div className="space-y-2">
                    <label className="text-[10px] md:text-xs font-bold tracking-wider text-gray-500 uppercase">Description</label>
            ...
            {adminTab === "music" && (
              <section className="bg-[#0a0a0a] border border-gray-800 p-4 md:p-10 rounded-3xl shadow-2xl relative">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 md:mb-8 gap-4">
                  <h2 className="text-xl md:text-2xl font-bold text-white">{editingMusicId ? "Edit Music" : "Add New Music"}</h2>
                  {editingMusicId && (
                    <button type="button" onClick={cancelEditMusic} className="text-sm bg-gray-800 text-gray-300 hover:text-white px-4 py-2 rounded-lg transition">
                      Cancel
                    </button>
                  )}
                </div>
                ...
                <form onSubmit={handleSaveMusic} className="space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                    <div className="space-y-2">
                      <label className="text-[10px] md:text-xs font-bold tracking-wider text-gray-500 uppercase">Track Title</label>
                      <input type="text" value={formMusicTitle} onChange={(e) => setFormMusicTitle(e.target.value)} required className="w-full bg-gray-950 border border-gray-800 p-3 rounded-xl focus:border-cyan-500 focus:outline-none transition text-white" />
                    </div>
                    <div className="space-y-2">
                      <label className="text-[10px] md:text-xs font-bold tracking-wider text-gray-500 uppercase">Artist Name</label>
                      <input type="text" value={formMusicArtist} onChange={(e) => setFormMusicArtist(e.target.value)} className="w-full bg-gray-950 border border-gray-800 p-3 rounded-xl focus:border-cyan-500 focus:outline-none transition text-white" />
                    </div>
                  </div>

                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                    <div className="space-y-2">
                      <label className="text-[10px] md:text-xs font-bold tracking-wider text-gray-500 uppercase">Listen Link (Spotify/SoundCloud/etc)</label>
                      <input type="url" value={formMusicUrl} onChange={(e) => setFormMusicUrl(e.target.value)} className="w-full bg-gray-950 border border-gray-800 p-3 rounded-xl focus:border-cyan-500 focus:outline-none transition text-white text-sm" />
                    </div>
                    <div className="space-y-2">
                      <label className="text-[10px] md:text-xs font-bold tracking-wider text-gray-500 uppercase">Release Date</label>
                      <input type="text" value={formMusicDate} onChange={(e) => setFormMusicDate(e.target.value)} className="w-full bg-gray-950 border border-gray-800 p-3 rounded-xl focus:border-cyan-500 focus:outline-none transition text-white text-sm" placeholder="e.g. 2024" />
                    </div>
                  </div>

                  <div className="space-y-2">
                    <div className="flex justify-between items-center">
                      <label className="text-[10px] md:text-xs font-bold tracking-wider text-gray-500 uppercase">Cover Art</label>
                      <button type="button" onClick={() => setMusicImageMode(musicImageMode === "url" ? "upload" : "url")} className="text-xs text-cyan-400 hover:underline">
                        {musicImageMode === "url" ? "Switch to Upload" : "Switch to URL"}
                      </button>
                    </div>
                    {musicImageMode === "url" ? <input type="url" value={formMusicCoverArt} onChange={(e) => setFormMusicCoverArt(e.target.value)} className="w-full bg-gray-950 border border-gray-800 p-3 rounded-xl focus:border-cyan-500 focus:outline-none transition text-white" placeholder="https://..." /> : <input type="file" accept="image/*" onChange={(e) => handleFileUpload(e, setFormMusicCoverArt)} className="w-full bg-gray-950 border border-gray-800 p-2 rounded-xl focus:outline-none file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-gray-800 file:text-white hover:file:bg-gray-700 text-gray-400 cursor-pointer" />}
                  </div>

                  <div className="pt-4">
                    <button type="submit" className="w-full md:w-auto px-8 bg-white hover:bg-gray-200 text-black font-bold py-3 rounded-xl transition shadow-lg">
                      {editingMusicId ? "Update Music" : "Publish Music"}
                    </button>
                  </div>
                </form>
              </section>
